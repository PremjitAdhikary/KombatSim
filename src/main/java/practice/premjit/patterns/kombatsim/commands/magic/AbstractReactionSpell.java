...
/**
 * Mages do not directly react to attacks on them. They do setup a defensive
 * aura or shield around them which deals with those attacks. To setup these
 * defenses, they cast a reaction spell. Once setup, the spell stays active for
 * a set duration during which it reacts to all incoming attacks.
 * <p>
 * 
 * So along with cool down timer, these spells have an additional switch timer
 * to track whether the spell is active or not. This is achieved by the
 * {@link AbstractReactionSpell#active active} {@link AbstractSpell.Switch
 * Switch}.
 * <p>
 * 
 * Other operations:
 ...
 * <li>Whether Spell {@link AbstractReactionSpell#canBeActivated()
 * canBeActivated()}
 * <li>{@link AbstractReactionSpell#activate() activate()} the Spell.
 * <li>Implement {@link AbstractReactionSpell#borrowFromSpellBook()
 * borrowFromSpellBook()} to invoke
 * {@link SpellBook#borrowSpell(AbstractReactionSpell)
 * book.borrowSpell(ReactionSpell)}
 * <li>Implement {@link AbstractReactionSpell#returnToSpellBook()
 * returnToSpellBook()} to invoke
 * {@link SpellBook#returnSpell(AbstractReactionSpell)
 * book.returnSpell(ReactionSpell)}
 * </ul>
 * 
