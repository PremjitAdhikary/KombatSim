...
/**
 * Along with base damage, this slows down the opponent. The slow down affect is based on opponent fighter class.<br><br> 
 * 
 * For example, most fighters are dexterity based, which means their actions are a function of their dexterity. For 
 * them their dexterity is lowered for a certain duration (which slows them down). <br><br>
 * 
 * Mages are an exception as they do not have dexterity and one of the attributes that they depend on to cast spell is 
 * enough mana. So for them, their mana pool is diminished for a certain duration. <br><br>
 * 
 * Visitor pattern {@link ColdDamage.ColdFighterVisitor} is applied to determine whether to affect dexterity or mana 
 * for reduction and restoration. <br><br>
 * 
 * Cold Deterioration can stack over another cold deterioration if the cold damages overlap <br><br>
 * 
