...
/**
 * Batman needs his custom strategy. He is the underdog against the giants like
 * Superman, Flash and other Mages.<br>
 * <br>
 * 
 * The vanilla Batman is most vulnerable, but agile. So while he can punch and
 * kick, the only attack he performs are kicks so as to impart the maximum
 * damage. <br>
 * <br>
 * 
 * The equipped Batman has augmented strength. So instead of kicks, he punches
 * around. <br>
 * The equipped Batman also has at his disposal a lot of {@link Projectiles
 * projectiles} that he can take advantage of. This strategy is to enable Batman
 * to use them effectively. <br>
 * <br>
 * 
 * The crux of the strategy for equipped Batman is that at regular interval to
 * weaken the opponent. And then inflict maximum damage by punching. <br>
 * Now every opponent has his own weakness. So choose the right projectile,
 * Batman first identifies his opponent. This is done by executing an initial
 * Move {@link Tracer}, which uses visitor pattern ({@link TracerVisitor}) to
 * determine the most effective projectiles against the opponent. Once we have
 * short listed the projectiles, one of them is selected at random and its
 * {@link ActionCommand command} executed. <br>
 * <br>
 * 
 * Along with that at fatal situations this Strategy also acts as a defensive
 * way where instead of attacking, Batman goes ahead and heals himself.
 * 
